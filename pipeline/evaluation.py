...
@torch.no_grad()
def collect_predictions(model, dataset, class_names, device, max_episodes: Optional[int] = None) -> Tuple[List, List, List]:
    """Predictions, ground truth and sampled classes of every episode, in dataset order."""
...
def evaluate(model, dataset, class_names, logger, device, max_episodes: Optional[int] = None) -> float:
    ...
    preds, gts, label2class = collect_predictions(model, dataset, class_names, device, max_episodes)
